...
class MemberListAdapter(
    ...
    override fun onBindViewHolder(viewHolder: MemberListViewHolder, position: Int) {
        try {
            ...
            viewHolder.binding.layoutMemberItem.setBackgroundColor(when {
                (0 == position % 2) -> ContextCompat.getColor(context, R.color.menu_item_background_even)
                else -> ContextCompat.getColor(context, R.color.menu_item_background_odd)
            })
