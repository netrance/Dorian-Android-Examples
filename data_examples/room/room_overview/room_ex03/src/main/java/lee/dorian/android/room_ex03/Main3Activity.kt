...
class Main3Activity : AppCompatActivity() {
    ...
    private val memberItemLongClickListener = OnMemberItemLongClickListener { v, member ->
        val popupMenu = PopupMenu(this@Main3Activity, v)
        menuInflater.inflate(R.menu.menu_member, popupMenu.menu)

        popupMenu.setOnMenuItemClickListener {
            when (it.itemId) {
                R.id.menuEdit -> {
                    val intent = Intent(this@Main3Activity, EditMember3Activity::class.java).apply {
                        putExtra(EditMember3ViewModel.KEY_MEMBER, member)
                    }
    ...
    private val flowInsertedRowIDCollector = FlowCollector<Long> {
        when (it) {
            -1L -> Toast.makeText(this@Main3Activity, "Cannot insert.", Toast.LENGTH_SHORT).show()
            else -> Toast.makeText(this@Main3Activity, "Inserted new member.", Toast.LENGTH_SHORT)
                .show()
        }
